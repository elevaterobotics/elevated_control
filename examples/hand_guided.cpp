...
void OnSignal(int /*signum*/) { g_running = false; }
}  // namespace
...
int main() {
  ...
  auto mode = arm.SwitchControlMode(elevated_control::ControlLevel::kHandGuided);
  if (!mode) {
    spdlog::error("Switch to hand-guided failed: {}", mode.error().message);
    arm.StopControlLoop();
    return EXIT_FAILURE;
  }
  ...
  spdlog::info("Hand-guided mode active; press Ctrl+C to exit");
